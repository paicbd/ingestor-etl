...
def read_from_pcap(pcap_file, pcap_hdr, endian, flatten):
    frame = 0
    frames = []
...
def read_from_pcapng(pcapng_file, pcap_hdr, endian, flatten):
    frame = 0
    frames = []
    ...
    block_len, = struct.unpack(endian + "I", pcap_hdr[4:8])
    section_hdr_block = pcap_hdr + pcapng_file.read(block_len - PCAP_GLOBAL_HDR_LEN)

    frames.append(section_hdr_block)
    ...
    frames.append(if_description_block)
    ...
    while True:
        try:
            ...
            block_type, block_len = struct.unpack(endian + "2I", block_hdr)
            if not block_type == 6:
                block_unk = block_hdr + pcapng_file.read(block_len - 8)
                frames.append(block_unk)
                continue
            ...
            # check if the block has Options
            options = b""
            if (len(packet) + 32) < block_len:
                options = enhanced_pkt_block[28 + header[2]:-4]
            ...
            if len(packet) == header[2]:
                if flatten:
                    if dlt_map[dlt][0](packet):
                        # if frame == 1:
                        #     print(len(packet))
                        pkts = flatten_sctp(dlt_map[dlt][1], packet)
                        if len(pkts) == 1:
                            frames.append(enhanced_pkt_block)
                            continue

                        # if frame == 1:
                        #     print(len(pkts[0]))
                        frames.extend(map((lambda p: struct.pack(endian + "I", block_type) +
                        struct.pack(endian + "I", 32 + len(p) + len(options)) +
                        enhanced_pkt_block[8:20] +
                        struct.pack(endian + "I", len(p)) +
                        struct.pack(endian + "I", len(p)) +
                        p +
                        options +
                        struct.pack(endian + "I", 32 + len(p) + len(options))),
            ...
            print(f"frame {frame} not processed of the PCAPNG file")
            continue
    log('n', f" read_pcap: read {frame} pkts, {len(frames)}")
    return frames

def get_frames(flatten):
    pcap_hdr = None
    frames = []

    with open(args.read_file, "rb") as pcap_file:
        pcap_hdr = pcap_file.read(PCAP_GLOBAL_HDR_LEN)
        if len(pcap_hdr) != PCAP_GLOBAL_HDR_LEN:
            raise Exception("read_pcap: cannot read header - file too short?")

        format, endian = get_format_and_endian(pcap_hdr)

        if format == "PCAP":
            frames = read_from_pcap(pcap_file, pcap_hdr, endian, flatten)
        elif format == "PCAPNG":
            frames = read_from_pcapng(pcap_file, pcap_hdr, endian, flatten)
...
log('q!', "Flattening")
pcap_hdr, frames = get_frames(args.flatten)
