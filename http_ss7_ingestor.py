...
import html
import struct
from models import HttpSS7, Base, IngestionQueue
import database
import xmltodict
import traceback
...
class TCPKey:
    ...
    def __str__(self):
        return f'TCPKey(src_ip={self.src_ip}, src_port={self.src_port}, dst_ip={self.dst_ip}, dst_port={self.dst_port}, ack_or_seq={self.ack_or_seq})'
...
def add_to_tcp_list(frame, dlt, header, packet):
    ...
    if m_req != None or m_res != None:
        ...
        if m_res == None:
            ...
            if temp == None:
                matching_key = TCPKey(tcp.dst_ip, tcp.dst_port, tcp.src_ip, tcp.src_port, tcp.seq_number + len(tcp.payload))
                incomplete_tcp_part = tcp_incomp_dict.get(matching_key)
                if incomplete_tcp_part is not None:
                    tcp.payload = tcp.payload + incomplete_tcp_part.payload
                    tcp.frames_list = tcp.frames_list + " " + incomplete_tcp_part.frames_list
...
def get_http(tcp: TCP):
    ...
    try:
        ...
        keys = http_dict.keys()
        if keys == None or len(list(keys)) == 0:
            return None
        ...
        match keys[0]:
            case "sriForSm":
                http.type = "sriForSm"
                try:
                    http.sccp_cd_adr = http_dict["sriForSm"]["sccpCdAdr"]["#text"]
                except Exception:
                    http.sccp_cd_adr = None
                
                try:
                    http.msisdn_orig = http_dict["sriForSm"]["msisdn"]["#text"]
                except Exception:
                    http.msisdn_orig = None
                
                try:
                    http.imsi = http_dict["sriForSm"]["imsi"]
                except Exception:
                    http.imsi = None
                
                try:
                    http.msc = http_dict["sriForSm"]["msc"]["#text"]
                except Exception:
                    http.msc = None
            ...
            case "smsmt":
                http.type = "smsmt"
                try:
                    http.imsi = http_dict["smsmt"]["imsi"]
                except Exception:
                    http.imsi = None
                
                try:
                    http.msc = http_dict["smsmt"]["msc"]["#text"]
                except Exception:
                    http.msc = None
                
                try:
                    http.msisdn_orig = http_dict["smsmt"]["orig"]["#text"]
                    http.msisdn_orig = html.unescape(http.msisdn_orig)
                except Exception:
                    http.msisdn_orig = None
                
                try:
                    udhi = http_dict["smsmt"]["smsDeliver"]["udhi"]
                    http.udhi = False
                    if udhi.lower() == "true":
                        http.udhi = True
                except Exception:
            ...
            case "smsmo":
                http.type = "smsmo"
                try:
                    http.msisdn_orig = http_dict["smsmo"]["orig"]["#text"]
                    http.msisdn_orig = html.unescape(http.msisdn_orig)
                except Exception:
                    http.msisdn_orig = None
                
                try:
                    http.msisdn_dest = http_dict["smsmo"]["dest"]["#text"]
                    http.msisdn_dest = html.unescape(http.msisdn_dest)
                    if len(http.msisdn_dest) > 32:
                        http.msisdn_dest = http.msisdn_dest[0:29] + "..."
                except Exception:
                    http.msisdn_dest = None
                
                try:
                    http.msc = http_dict["smsmo"]["msc"]["#text"]
                except Exception:
                    http.msc = None
                
                try:
                    http.imsi = http_dict["smsmo"]["imsi"]
                except Exception:
                    http.imsi = None
                
                try:
                    udhi = http_dict["smsmo"]["smsSubmit"]["udhi"]
                    http.udhi = False
                    if udhi.lower() == "true":
                        http.udhi = True
                except Exception:
                ...
                try:
                    http.session_id = http_dict["smsmo"]["sessionId"]
                except Exception:
                    http.session_id = None
                
                
            case "alertSC":
                http.type = "alertSC"
                try:
                    http.msisdn_orig = http_dict["alertSC"]["msisdn"]["#text"]
                    http.msisdn_orig = html.unescape(http.msisdn_orig)
                except Exception:
                    http.msisdn_orig = None
                
                
            case "reportSMDeliver":
                http.type = "reportSMDeliver"
                try:
                    http.sccp_cd_adr = http_dict["reportSMDeliver"]["sccpCdAdr"]["#text"]
                except Exception:
                    http.sccp_cd_adr = None
                
                try:
                    http.msisdn_orig = http_dict["reportSMDeliver"]["msisdn"]["#text"]
                    http.msisdn_orig = html.unescape(http.msisdn_orig)
                except Exception:
                    http.msisdn_orig = None
            ...
            case "sms":
                http.type = "sms"
                try:
                    http.msisdn_orig = http_dict["sms"]["orig"]["#text"]
                    http.msisdn_orig = html.unescape(http.msisdn_orig)
                except Exception:
                    http.msisdn_orig = None
                
                try:
                    http.imsi = http_dict["sms"]["imsi"]
                except Exception:
                    http.imsi = None
                
                try:
                    udhi = http_dict["sms"]["smsDeliver"]["udhi"]
                    http.udhi = False
                    if udhi.lower() == "true":
                        http.udhi = True
                except Exception:
            ...
            case "error":
                http.type = "error"
                try:
                    http.text = http_dict["error"]["text"]
                except Exception:
                    http.text = None
            case _:
                pass
    ...
    for k in tcp_global_dict.keys():
        ...
        if v_list != None:
            ...
            if v_list[0] != None and v_list[1] != None:
                http_req = get_http(v_list[0])
                http_res = get_http(v_list[1])
                if http_req != None and http_res != None:
                    # if the request exists, then copy MSISDN and IMSI between request and response
                    # MSISDN ORIG
                    if http_req.msisdn_orig != None and http_req.msisdn_orig != "":
                        if http_res.msisdn_orig == None or http_res.msisdn_orig == "":
                            http_res.msisdn_orig = http_req.msisdn_orig
                    else:
                        if http_res.msisdn_orig != None and http_res.msisdn_orig != "":
                            http_req.msisdn_orig = http_res.msisdn_orig
                    
                    # MSISDN DEST
                    if http_req.msisdn_dest != None and http_req.msisdn_dest != "":
                        if http_res.msisdn_dest == None or http_res.msisdn_dest == "":
                            http_res.msisdn_dest = http_req.msisdn_dest
                    else:
                        if http_res.msisdn_dest != None and http_res.msisdn_dest != "":
                            http_req.msisdn_dest = http_res.msisdn_dest
                ...
                if v_list[0] != None:
                    http = get_http(v_list[0])
                    if http != None:
                        http_unlink_list.append(http)
                
                if v_list[1] != None:
                    http = get_http(v_list[1])
                    if http != None:
                        http_unlink_list.append(http)
